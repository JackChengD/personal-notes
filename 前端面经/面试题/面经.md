...
```javascript
    ...
    var twoSum = function (nums, target) {
      ...
      for(let i=0;i<nums.length;i++){
        const cur = nums[i];
        const curReduce = target - nums[i];
        if(curReduce in obj) return [obj[curReduce],i];
        obj[cur] = i;
      }
    };
...
>- absolute相对最近的一级relatve、absolute
>- fixed相对浏览器窗口会随着浏览器滚动而滚动
>- static默认值没有定位，元素出现在正常的流中

7.http的头部

1. http请求头部有Cookie、Contection、Content-Type、Date、If-Modified-Since、If-None-Match、user-agent
2. http响应头部有Set-Cookie、Cache-Control、Content-Language、Content-Length、Date、Content-Type、Last-Modified、ETag、Expires
...
```javascript
    ...
    //12. mounted el被新创建的vm.$el替换，并挂载到实例上去之后调用该钩子，如果root实例挂载了一个文档内元素，当mounted被调用时vm.$el也在文档内
...
>- 一般来说，v-if有更大得切换开销，而v-show有更高得初始渲染开销。因此，如果需要非常频繁地切换，则使用v-show较好，如果在运行时条件很少改变，则使用v-if较好
...
```javascript
    ...
    function fn(arr) {
      return arr.reduce((prev, cur) => {
        return Array.isArray(cur) ? prev.concat(fn(cur)) : prev.concat(cur)
      }, []);
    }
```
...
> 单指针也可以，使用标记法，对每个节点都标记，然后遍历的时候看有没有标记，有的话就是循环链表  
...
>- 4.componentUpdated：指令所组件的VNode其子VNode全部更新后调用
...
>- 4.oldVnode：上一个虚拟节点，��在update和componentUpdated钩子中可用
...
>- 1. 更利于SEO
>
>- >- 不同爬虫工作原理类似，不会爬取源码，不会执行网站的任何脚本(Google除外，据说Googlebot可以运行JavaScript)。使用了React或者其他MVVM框架之后，页面大多数DOM元素都是在客户端根据JS动态生成，可供爬虫爬取分析的内容大大较少。另外，浏览器爬虫不会等待我们的数据完成之后再去爬取我们的页面数据。服务端渲染返回给客户端的是已经获取了异步数据并执行JavaScript脚本的最终html，网络爬虫就可以爬取到完整的页面
>
>- 2. 更利于首屏渲染
>
...
5. 总结
>
>- 服务端渲染利于SEO搜索引擎优化，服务端渲染是可以被爬虫爬取到的，客户端异步渲染是很难被爬虫爬取到的，所以你会发现真正的网站既不是纯异步也不是纯服务端渲染出来的，而是两者结合来做的。例如京东的商品列表就采用服务端渲染，目的为了SEO搜索引擎优化，而它的商品评论列表为了用户体验，而且也不需要SEO优化，所以采用客户端渲染
...
> 参考： <https://juejin.im/post/5ec78921f265da77057e23a4#heading-2>
...
> 3. 为了插队，微任务会在下一个event loop之前执行调完
...
2.vue的异步更新视图
> 比如，数据改变了什么会异步修改视图，同步不行吗？  
> Vue的dom更新是异步的，当数据发生变化，vue并不是离开更新dom，而是开启一个队列。跟JavaScript原生的同步任务和异步任务相同。
> Vue在一个同步任务中是不会去更新渲染视图，而是在同步任务（事件循环队列）执行完毕之后，在主线程的同步任务执行完毕，读取任务队列时更新视图。  
> 这个机制对于页面性能是非常重要的，因为我们要是每操作一个数据就更新一个视图，那么性能会非常的差，而如果是一次任务执行完毕之后更新视图，可以避免特别多的重复操作
> 在开发过程中，我们很容易遇见需要先渲染然后操作dom，这时候就要使用vue提供的nextTick函数。
> 这个函数就是有数据之后，渲染完成之后会执行，也就是说当你需要数据先渲染，然后去操作渲染完成之后的dom，要把操作dom的逻辑放在nextTick函数里面。
...
> 2）操作系统缓存：如果浏览器缓存中找不到需要的DNS记录，就会去操作系统中找hosts文件（Windows环境下，维护一张域名与IP地址的对应表，位置一般C:\Windows\System32\drivers\etc\hosts）  
> 3）路由缓存：路由器也有DNS缓存  
...
> 浏览器创建自定义的解析器来解析HTML，解析算法分为标记化和树构建。标记化是词法分析过程中，将输入内容解析成多个标记。HTML标记包括起始标记、结束标记、属性名称和属性值。标记生成器识别标记，传递给树构造器，然后接受下一个字符以识别下一个标记；直到输入的结束。  
> 参考：<https://blog.csdn.net/greenqingqingws/article/details/19163061>  
> 参考：<https://juejin.im/post/5eba6e3a6fb9a04346736a53>  
...
```javascript
  function fn(){
    ...
    let curTemp;
    for(let i=0;i<len;i++){
        curTemp = Math.floor(Math.random()*10000+1);
        curArr.push(curTemp);
    }
    let newArr = [...new Set(curArr)].sort((a,b)=>a-b);
    return newArr;
  }
  fn();
...
```javascript
    function fn(str){
      ...
      if (str != num.toString() || num >= 100000 || num <= 0) {
        ...
        console.log('str1111',str.split('').map(Number).reverse())
        str = '10000'
...
5.options，为什么要有options

6.content-type是干嘛的，谁定义的
...
3.eventLoop以及题目（promise、async、setTimeout）

4.es6新特性（let/const、箭头函数、解构、Set/Map/WeakSet/WeakMap、proxy、reflect、generator、Symbol。。。）

5.出入栈算法

6.判断链表是环、长度、倒数第k个数算法

7.树的层次遍历算法

8.用两个栈实现一个队列

9.url渲染过程（缓存、nds解析、http与https区别（怎么验证证书）、tcp手次握手）
...
3.8个球，一个稍重，至少通过多少步天平找到稍重的球

参考： https://blog.csdn.net/ACBattle/article/details/98474269

4.判断数组B是否为数组A的子数组

5.找到数组中值大于前面，小于后面的集合
...
1. 热重载：重新加载，使用window.location.reload()，刷新界面以达到更新的目的
2. 热更新：只更新修改的文件。不会刷新界面

   1. 启动webpack服务器，与浏览器建立websocket通信，webpack每次打包编译后，会新生成hash、json、js文件，
   2. webpack服务器会给浏览器传hash，浏览器对比hash是否发生变化
   3. 如果变化了就会向webpack服务器请求变化的内容hot-update.json，获取当前变化的文件chunkId。
   4. 然后chunkId，去拿到更新后的内容hot-update.js
   5. 浏览器替换当前chunkId的内容并执行当前新的chunk代码，触发页面的更新
hash是新的hash值，用来对比前后hash是否变化
json是记录本次修改的模块
js是本次修改的具体内容

参考：https://juejin.cn/post/6976082291428704292

4.一架手机里面的一个动态效果怎么实现，里面的性能标杆  
链接：https://www.oneplus.com/cn/ace-pro?from=head
...
1. 会有浏览器兼容问题，vite需要在支持原生esmodule模块动态导入的浏览器中使用，可以引入官方插件（@vitejs/plugin-legacy）解决
2. 第三方插件需要支持esmodule
3. vite是当访问的时候再对需要的文件进行编译打包，那么首屏的耗时就会提升，直接关系到用户的体验（比如webpack全量打包100s，vite启动1s，首屏模块打包要20s，那么就会白屏20s）

8.vite打包和rollup打包有什么区别(vite需要加强)

9.编码

是信息从一种形式或格式转换为另外一种形式的过程。
常见的有UTF8、ASCII、GBK
...
题目：
给数据，里面有id、pid、time，相互直接有关系，pid是id的父亲，time是执行的时间。id的执行依赖pid，计算整个链路执行的最短时间。
...
function fn1(arr) {
    ...
    // 递归找到每一层的最大值
    function fn2(source, curSum) {
        let result = 0;
        for (let i = 0; i < source.length; i++) {
            let sourceSum = 0;
            if (source[i].children) {
                sourceSum = fn2(source[i].children, curSum + source[i].time);
            } else {
                sourceSum = curSum + source[i].time;
            }
            result = Math.max(result, sourceSum)
        }
        return result;
    }

    result = fn2(timeArr, 0);
...
head头循序
1、开启dns预获取
2、cos资源预连接 preconnect
3、cos资源预获取 dns-prefetch
4、图片资源做dns预获取 dns-prefetch
5、设置<meta name="csrf-token" content="<%=csrfToken%>">，预防csrf
6、设置<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />，规定文档解析
7、设置<meta name="renderer" content="webkit">，默认使用webkit（国内浏览器双核webkit、ie）
8、link preload预加载头图
9、link canoical url
10、script schema 
11、link alternate 当前页面站点语言版本
12、数据上报、监控上报 dns预获取 dns-prefetch
13、设置keywords、description
14、设置og设置
15、通用全局样式
...
csr（客户端渲染）表示所有的页面渲染（render）都透过浏览器端的js来完成，所有的逻辑、取资料、路由、template都在客户端处理。
优点是
更好的交互体验，页面的更新或者换页都不需要刷新页面，在使用体验上相较于传统的ssr应用会顺畅很多，大多数交互都不要刷新页面

缺点主要两点：
1、载入速度通常较慢，csr要等待js下载及执行render，所有csr页面载入的前几秒，页面上会没有东西或者只有一些骨架，使用者必须要等待一段时间才能看到页面的内容，相对来说会是较差的使用者体验。
2、seo会较差，因为csr的页面对于爬虫是比较不友善的，虽然爬虫有办法执行js，但是爬虫也有一些js render页面的限制。
...
ssr（服务器渲染）表示服务器收到使用的请求之后，在服务器生成完成的html，因为生成html的时候会在服务器端先获取内部或者外部api资料，所以相较于csr从浏览器端获取资料的模式，ssr可以省去多次的来回往返时间。
在服务器render完html传递给使用者，这时候已经有完整的html，所以很快就来到fcp（first contentful paint，使用者首次看到页面上重要内容的时间。）
接下来下载并且执行浏览器互动所需要的js，完成后来到了tti（time to interactive）使用者首次可以跟页面互动的时间点。

优点：
1、需要的js比较少，因此有较快的tti，可以较快开始交互，原因是相较于csr所有render的逻辑都包含在js中，ssr已经在服务端把render工作做完了，ssr方案中浏览器需要的js理论上会比较少，所以会比较快达到tti。
2、有更多的js预算可以留给其他第三方js
3、seo较佳，因为ssr产生的完整html可以很容易的被爬虫解读，不需要想办法执行js，seo也是大部分的人会考虑ssr方案的最主要原因之一。

缺点：
1、较慢的ttfb（time to first byte，从浏览页面的动作开始到浏览器收到第一个byte所需要的时间）。因为在服务端生成完整的html很花时间。如果同时有许多人造访server造成负担很重，或是有一些非常慢的api，都有可能让server的回应非常慢。
2、互动性体验差，因为ssr的页面在每次互动 之间都要重新读取页面，这在使用体验上就不如csr的页面流畅，也是现代web app大多数会采用csr方案的主要原因。
...
ssr更利于首屏的渲染，csr更利于用户的交互

3、vite、webpack和gulp区别  
gulp是一个基于流的构建工具，可以自动执行指定的任务，简洁且高效，没有模块化

webpack是模块化管理工具，使用webpack可以对模块进行压缩、预处理、按需打包、按需加载等。

Gulp  
定位： 基于流的自动化构建工具  
目标： 自动化和优化开发工作流，为通用 website 开发而生  
学习难度： 易于学习，易于使用，api总共只有5个方法  
适用场景： 基于流的作业方式适合多页面应用开发
作业方式： 对输入（gulp.src）的 js，ts，scss，less 等源文件依次执行打包（bundle）、编译（compile）、压缩、重命名等处理后输出（gulp.dest）到指定目录中去，为了构建而打包  
使用方式： 常规 js 开发，编写一系列构建任务（task）。
优点： 适合多页面开发，易于学习，易于使用，接口优雅。  
缺点： 在单页面应用方面输出乏力，而且对流行的单页技术有些难以处理（比如 Vue 单文件组件，使用 gulp 处理就会很困难，而 webpack 一个 loader 就能轻松搞定）  
结论：浏览器多页应用(MPA)首选方案

webpack
定位： 通用模块打包加载器，为移动端大型 SPA 应用而生  
学习难度： 有大量新的概念和api，不过好在有详尽的官方文档  
适用场景： 一切皆模块的特点适合单页面应用开发  
作业方式： 对入口文件（entry）递归解析生成依赖关系图，然后将所有依赖打包在一起，在打包之前会将所有依赖转译成可打包的 js 模块，为了打包而构建  
使用方式： 编辑各种 JSON 配置项
优点： 可以打包一切资源，适配各种模块系统
缺点： 不适合多页应用开发，灵活度高但同时配置很繁琐复杂。“打包一切” 这个优点对于 HTTP/1.1 尤其重要，因为所有资源打包在一起能明显减少浏览器访问页面时的资源请求数量，从而减少应用程序必须等待的时间。但这个优点可能会随着 HTTP/2 的流行而变得不那么突出，因为 HTTP/2 的多路复用可以有效解决客户端并行请求时的瓶颈问题。
结论：浏览器单页应用(SPA)首选方案  
...
6、有没有针对代码的利用率做优化
chrome浏览器，打开f12，通过ctrl+shift+p 输入 coverage可以查看unused bytes  
https://juejin.cn/post/6844904023905026055

7、ssr的构建过程

使用vue-server-renderer

- 配置client、server不同入口文件，通过webpack的不同配置文件，打包出client bundler和server bundler  
- server bundler运行再node端，将渲染好的Vue实例转换成HTML返回给客户端，而client bundler会将返回的HTML使其转变为由Vue管理的动态Dom
- 使用vue-server-renderer提供的插件，可打包服务端和客户端的构建清单，通过该清单，可以自动向要返回的HTML中自动注入客户端打包好的js文件

参考：https://juejin.cn/post/6867330846194892808#heading-9  
参考：https://cn.vuejs.org/guide/scaling-up/ssr.html#what-is-ssr
...
- 垄断，接口异常几分钟挂了多少次，就认为页面挂了
- 重试，几分钟后再重试一次
- 缓存，redis缓存上一次结果
- 告警

3、h5内嵌客户端做上报，webview

- 合理的做法是，通过桥接把数据给客户端，由客户端做上报
...
图片：根据网络、设备展示多倍图，预加载，懒加载，gif转静态图，雪碧图，压缩
...
11、service work做极致的性能优化

12、怎么做性能的数据采集

13、web-vitals

在client入口做上报，通过web-vitals拿到LCP,CLS,FID的方法，传入回调函数拿到性能的infos数据（name，delta，value）对不同指标（lcp,cls,fid）做上报
也会上报用户ua，页面url，设备，网络情况等
参考：https://juejin.cn/post/7083301090707374087
...
6、组件封装
思考点:
怎么让不同端都能使用(小程序,vue2,vue3,react等),不同端的请求/存储/模块支持等都不一样
项目升级
...
1.0.1+0.2!=0.3的原理，如何解决
因为计算机是采用二进制来表示十进制的，规则是：整数除以2，商继续除以2，得到0为止，将余数逆序排列；小数乘以2，取整，小数部分继续乘以2，取整，得到小数部分0为止，将整数顺序排列。  

解决：  
parseFloat((0.1+0.2).toFixed(10))===0.3  

十进制传二进制，二进制相加，转成十进制导致不等。
...
参考：<https://juejin.cn/post/6844903988584775693>

7.怎么阻止事件冒泡
e.stopPropagation()

8.怎么取消默认事件
默认事件是浏览器自带的事件，比如鼠标右键菜单。  
使用e.preventDefault();取消默认事件
...
10.前后端分离部署，部署时间不一致或接口更改导致的问题如何处理
一般情况下不会产生这个问题，毕竟部署都是半夜一起部署的，有问题的话就会及时更改。怕导致有问题的话就可以按蓝绿部署、滚动部署或者灰度部署

参考：<https://www.jianshu.com/p/9d417ed2fa21>

## 百度

1.数组中一万个数据,访问第一个和最后一个效率会有什么差异,为什么
访问的话没有区别，数组是一块线性连续的内存,我们可以通过寻址公式一步取出对应的成员，这跟成员的位置没有关系  
参考：<https://www.cnblogs.com/duxinyi/p/11491552.html>

2.什么是迭代器  
能够用for循环的对象成为迭代对象  

3.为什么js是单线程  
js主要是实现用户与浏览器的交互，以及操作dom；这决定了它只能是单线程，否则会带来很复杂的同步问题。举个例子：如果js被设计了多线程，如果有一个线程要修改一个dom元素，另一个线程要删除这个dom元素，此时浏览器就会一脸茫然，不知所措。  

参考：<https://www.jiangweishan.com/article/js20209871132132132.html>

4.Object.create(null)  
创建一个很纯净的对象，里面什么都没，比如说，我只需要用对象来保存一些数据，然后进行循环取用，提高循环效率。这个时候如果对象有原型链，那便会在循环的时候去循环它的各个属性和方法，然后这不是必要的，我们只是要他里面的元素而已，效率会降低  
...
参考： <http://shouce.jb51.net/js-function/ch4.html>
