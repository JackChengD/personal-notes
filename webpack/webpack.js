...
class Compiler {
  ...
  compile(callback) {
    ...
    let compalation = new Compilation(this.options);
    compalation.build(callback); // 执行compilation的build方法进行编译，编译成功之后执行回调
  }
  ...
  run(callback) {
    ...
    const onCompiled = (err, stats, fileDependencies) => {
      ...
      // 对 this.fileDependencies 里面的文件进行监听，当文件发生变化时，重新执行 compile 函数
...
class Compilation {
      ...
      CallExpression: (NodePath) => {
        ...
        if (node.callee.name === "require") {
          ...
          let extensions = (this.options.resolve &&
            this.options.resolve.extensions) || [".js"];
  ...
  build(callback) {
    ...
    for (let entryName in entry) {
      ...
      // 6.1 把入口文件的绝对路径添加到依赖数组（`this.fileDependencies`）中
      // 记录此次编译依赖的模块
      ...
      let chunk = {
        // entryName="main" 代码块的名称
        name: entryName,
        // 此代码块对应的module的对象,这里就是src/index.js 的module对象
        entryModule,
        // 找出属于该代码块的模块
